...
class HealthAppState extends State<HealthApp> {
  ...
  Future<void> addData() async {
    ...
    success &=
        await health.writeHealthData(value: 1.925, type: HealthDataType.HEIGHT, startTime: earlier, endTime: now, recordingMethod: RecordingMethod.manual);
    ...
    success &=
        await health.writeHealthData(value: 90, type: HealthDataType.HEART_RATE, startTime: earlier, endTime: now, recordingMethod: RecordingMethod.manual);
    success &= await health.writeHealthData(value: 90, type: HealthDataType.STEPS, startTime: earlier, endTime: now, recordingMethod: RecordingMethod.manual);
    ...
    success &= await health.writeHealthData(value: 22, type: HealthDataType.LEAN_BODY_MASS, startTime: earlier, endTime: now);
    ...
    if (Platform.isIOS) {
      success &= await health.writeHealthData(
          value: 22, type: HealthDataType.WATER_TEMPERATURE, startTime: earlier, endTime: now, recordingMethod: RecordingMethod.manual);

      success &= await health.writeHealthData(
          value: 55, type: HealthDataType.UNDERWATER_DEPTH, startTime: earlier, endTime: now, recordingMethod: RecordingMethod.manual);
      success &=
          await health.writeHealthData(value: 4.3, type: HealthDataType.UV_INDEX, startTime: earlier, endTime: now, recordingMethod: RecordingMethod.manual);
    }
  ...
  /// Opens Health Connect onboarding screen
  Future<void> openHealthConnectOnboarding() async {
    bool success = await health.openHealthConnectScreen();
    debugPrint("Health Connect Onboarding opened: $success");
  }
  ...
  Future<void> getIntervalBasedData() async {
    ...
    debugPrint('Total number of interval data points: ${healthDataResponse.length}. '
              ...
              children: [
                ...
                if (Platform.isIOS || Platform.isAndroid && health.healthConnectSdkStatus == HealthConnectSdkStatus.sdkAvailable)
                  Wrap(spacing: 10, children: [
                    ...
                    if (Platform.isAndroid)
                      TextButton(
                          onPressed: openHealthConnectOnboarding,
                          style: const ButtonStyle(backgroundColor: WidgetStatePropertyAll(Colors.blue)),
                          child: const Text("HC Onboarding", style: TextStyle(color: Colors.white))),
                  ]),
              ],
            ),
