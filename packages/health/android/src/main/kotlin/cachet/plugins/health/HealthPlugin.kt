...
class HealthPlugin(private var channel: MethodChannel? = null) :
    ...
    private fun getHealthDataValue(dataPoint: DataPoint, field: Field): Any {
        ...
        return when (value.format) {
            Field.FORMAT_FLOAT ->
                    if (!isGlucose) value.asFloat() else value.asFloat() * MMOLL_2_MGDL
        ...
        try {
            ...
            Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount)
                    ...
                    .addOnFailureListener(
                            errHandler(result, "There was an error deleting the dataset")
                    )
        } catch (e3: Exception) {
            result.success(false)
        }
    ...
    private fun writeMeal(call: MethodCall, result: Result) {
        ...
        try {
            ...
            Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount)
                    ...
                    .addOnFailureListener(
                            errHandler(result, "There was an error adding the meal data!")
                    )
        } catch (e3: Exception) {
            result.success(false)
        }
    ...
    private fun writeData(call: MethodCall, result: Result) {
        ...
        val dataPoint =
                if (!isIntField(dataSource, field)) {
                    builder.setField(
                                    field,
                                    (if (!isGlucose) value else (value / MMOLL_2_MGDL).toFloat())
                            )
                            .build()
                } else {
        ...
        try {
            ...
            Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount)
                    ...
                    .addOnFailureListener(
                            errHandler(result, "There was an error adding the dataset")
                    )
        } catch (e3: Exception) {
            result.success(false)
        }
    ...
    private fun writeWorkoutData(call: MethodCall, result: Result) {
        ...
        val session =
                Session.Builder()
                        .setName(
                                activityType
                        ) // TODO: Make a sensible name / allow user to set name
                        .setDescription("")
        ...
        try {
            ...
            Fitness.getSessionsClient(
                    ...
                    .addOnFailureListener(
                            errHandler(result, "There was an error adding the workout")
                    )
        } catch (e: Exception) {
            result.success(false)
        }
    ...
    private fun getIntervalData(call: MethodCall, result: Result) {
        ...
        Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount)
                ...
                .addOnSuccessListener(
                        threadPoolExecutor!!,
                        intervalDataHandler(dataType, field, includeManualEntry, result)
                )
                .addOnFailureListener(
                        errHandler(result, "There was an error getting the interval data!")
                )
    }
    ...
    private fun getAggregateData(call: MethodCall, result: Result) {
        ...
        Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount)
                ...
                .addOnFailureListener(
                        errHandler(result, "There was an error getting the aggregate data!")
                )
    }
    ...
    private fun dataHandler(
        ...
        /// For each data point, extract the contents and send them to Flutter, along with date and
        // unit.
                ...
                for (session in response.sessions) {
                    ...
                    if (type == SLEEP_ASLEEP) {
                        healthData.add(
                                hashMapOf(
                                        "value" to
                                                session.getEndTime(TimeUnit.MINUTES) -
                        ...
                        if (dataSets.isNotEmpty()) {
                            for (dataSet in dataSets) {
                                for (dataPoint in dataSet.dataPoints) {
                                    ...
                                    if (dataPoint
                                                    .getValue(Field.FIELD_SLEEP_SEGMENT_TYPE)
                                                    .asInt() != 3
                                    ) {
                                        healthData.add(
                                                hashMapOf(
                            ...
                            for (dataPoint in dataSet.dataPoints) {
                                ...
                                if (dataPoint.getValue(Field.FIELD_SLEEP_SEGMENT_TYPE).asInt() == 1
                                    ...
                                    healthData.add(
                                            hashMapOf(
                                                    "value" to
                                                            dataPoint.getEndTime(TimeUnit.MINUTES) -
                                                                    dataPoint.getStartTime(
                                                                            TimeUnit.MINUTES,
                                                                    ),
                                                    "date_from" to
                                            ...
                                            "source_name" to
                                                    (dataPoint.originalDataSource.appPackageName
                                                            ?: (dataPoint
                                                                    ...
                                                                    ?: "")),
                                            "source_id" to
                                                    dataPoint.originalDataSource.streamIdentifier,
                                            "is_manual_entry" to
                                                    dataPoint.originalDataSource.streamName
                                                            .contains("user_input")
                                    )
                                }
    ...
    private fun aggregateDataHandler(includeManualEntry: Boolean, result: Result) =
            OnSuccessListener { response: DataReadResponse ->
                ...
                for (bucket in response.buckets) {
                    ...
                    /// Fetch all data points for the specified DataType
                    for (dataSet in bucket.dataSets) {
                        /// For each data point, extract the contents and send them to Flutter,
                    ...
                    val healthDataItems =
                            hashMapOf(
                                    "value" to
                                            bucket.getEndTime(TimeUnit.MINUTES) -
                                                    bucket.getStartTime(TimeUnit.MINUTES),
                    ...
                    healthData.add(
                            hashMapOf(
                                    "workoutActivityType" to
                                            (workoutTypeMap
                                                    .filterValues { it == session.activity }
                                                    .keys
                                                    .firstOrNull()
                                                    ?: "OTHER"),
                                    ...
                                    "totalDistance" to
                                            if (totalDistance == 0.0) null else totalDistance,
            ...
            MapToHCType[dataType]?.let { classType ->
                ...
                var request =
                        ReadRecordsRequest(
                                ...
                                // Define the maximum amount of data that HealthConnect can return
                                // in a single request
            ...
            is SleepSessionRecord ->
                    return listOf(
                            mapOf<String, Any>(
                                    ...
                                    "source_name" to metadata.dataOrigin.packageName,
                                    "stage" to
                                            if (record.stages.isNotEmpty()) record.stages[0]
            ...
            is NutritionRecord ->
                    return listOf(
                            mapOf<String, Any>(
                                    ...
                                    "name" to record.name!!,
                                    "mealType" to
                                            (MapTypeToMealTypeHC[record.mealType]
                                                    ?: MEAL_TYPE_UNKNOWN),
                                    "date_from" to record.startTime.toEpochMilli(),
                                    "date_to" to record.endTime.toEpochMilli(),
                                    "source_id" to "",
    ...
    // TODO rewrite sleep to fit new update better --> compare with Apple and see if we should not
