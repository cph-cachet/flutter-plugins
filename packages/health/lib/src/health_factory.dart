...
class HealthFactory {
  ...
  Future<List<HealthDataPoint>> _computeAndroidBMI(DateTime startDate, DateTime endDate) async {
    List<HealthDataPoint> heights = await _getSingleDataPoint(startDate, endDate, HealthDataType.HEIGHT);
    List<HealthDataPoint> weights = await _getSingleDataPoint(startDate, endDate, HealthDataType.WEIGHT);
    ...
    for (int i = 0; i < weights.length; i++) {
      ...
      print('BMI: $bmiValue');
      HealthDataPoint x = HealthDataPoint._(bmiValue, HealthDataType.BODY_MASS_INDEX, unit, weights[i].dateFrom, weights[i].dateTo, _platformType, _deviceId);
  ...
  Future<List<HealthDataPoint>> getHealthDataFromTypes(DateTime startDate, DateTime endDate, List<HealthDataType> types) async {
    ...
    for (HealthDataType type in types) {
      List<HealthDataPoint> result = await _getSingleDataPoint(startDate, endDate, type);
  ...
  Future<List<HealthDataPoint>> _getSingleDataPoint(DateTime startDate, DateTime endDate, HealthDataType dataType) async {
