...
class HealthFactory {
  ...
  Future<bool> writeMenstrualFlow(
    ...
    if (_platformType != PlatformType.IOS) throw ArgumentError("MenstrualFlow is only supported in HealthKit");
    Map<String, dynamic> args = {
      'flow': flow.index,
  ...
  static List<HealthDataPoint> _parse(Map<String, dynamic> message) {
    ...
    final list = dataPoints.map<HealthDataPoint>((e) {
      ...
      HealthValue value;
      if (dataType == HealthDataType.MENSTRUAL_FLOW) {
        value = MenstrualFlowHealthValue.fromJson(e);
      } else if (dataType == HealthDataType.AUDIOGRAM) {
